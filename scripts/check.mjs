...
for (const file of htmlFiles) {
  ...
  for (const match of html.matchAll(/(?:href|src)="(\/escala-clara\/[^"#?]*)/g)) {
    let target = match[1].replace('/escala-clara', '') || '/index.html';
...
for (const required of ['plano-a-real/', 'hallar-escala/', 'reescalar-impresion/']) if (!sitemap.includes(required)) failures.push(`sitemap: falta ${required}`);
